...
WITH source_data AS (
    SELECT * FROM {{ source('bronze_saleslt', 'productmodelproductdescription') }}
)
...
SELECT
    {% for col in adapter.get_columns_in_relation(source('bronze_saleslt', 'productmodelproductdescription')) %}
