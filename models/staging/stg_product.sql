...
-- CTE to highlight source from yml file
WITH source_data AS (
    SELECT * FROM {{ source('bronze_saleslt', 'product') }}
)
...
SELECT
    {% for col in adapter.get_columns_in_relation(source('bronze_saleslt', 'product')) %}
    -- checks date in column name and then applies clean date macro
        {% if 'date' in col.name | lower %}
            {{ clean_date(col.name) }} AS {{ col.name }}
            
    -- transforms decimal numbers to two decimal points in columns StandardCost and ListPrice
        {% elif col.name in ('StandardCost', 'ListPrice') %}
